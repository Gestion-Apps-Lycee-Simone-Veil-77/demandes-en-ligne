...
export function buildRecapDepense(row) {
  return [
    ...
    { label: 'Demandeur.euse', value: `${row.Prenom} ${row.Nom} (${row.Email})` },
...
export function buildRecapSalle(row) {
  ...
  return [
    ...
    { label: 'Date de saisie', value: formatDateTimeFr(row.DateSaisie) },
    { label: 'Demandeur.euse', value: `${row.Prenom} ${row.Nom} (${row.Email})` },
    { label: 'Téléphone du demandeur.euse', value: row.DemandeurTelephone },
    { label: 'Activité / fonction du demandeur.euse', value: row.DemandeurActivite },
    { label: 'Intitulé de la formation', value: row.IntituleFormation },
    { label: 'Référent.e de la formation', value: row.ReferentNom },
    { label: 'Téléphone du référent.e', value: row.ReferentTelephone },
    { label: 'Email du référent.e', value: row.ReferentEmail },
    ...
    { label: 'Formateur.ices - Fonction', value: row.Formateurs },
...
export function buildRecapSortie(row) {
  ...
  return [
    { label: 'N° de demande', value: row.NumeroRequest },
    { label: 'Date de saisie', value: formatDateTimeFr(row.DateSaisie) },
    { label: 'Demandeur.euse (compte connecté)', value: `${row.Prenom} ${row.Nom} (${row.Email})` },
    { label: 'Professeur.e organisateur.ices', value: row.ProfesseurOrganisateur },
    { label: "Email de l'organisateur.ices", value: row.EmailOrganisateur },
    { label: "Organisateur.ices — cours à banaliser", value: formatCoursBanalises(String(row.OrganisateurCoursBanalises || '').split(',').filter(Boolean)) },
    ...
    {
      label: 'Accompagnateur.ices (cours à banaliser)',
      value: accompagnateurs.length
        ? accompagnateurs.map(a => `${a.nom} — ${formatCoursBanalises(a.coursBanalises)}`).join(' ; ')
        : '—'
    },
    { label: "Billets d'entrée accompagnateur.ices — nombre", value: row.NbBilletEntreeAccompagnateurs },
    { label: "Billets d'entrée accompagnateur.ices — coût individuel", value: row.CoutBilletEntreeAccompagnateurs ? `${formatMontant(row.CoutBilletEntreeAccompagnateurs)} €` : '—' },
    { label: 'Tickets de transport accompagnateur.ices — nombre', value: row.NbTicketTransportAccompagnateurs },
    { label: 'Tickets de transport accompagnateur.ices — coût individuel', value: row.CoutTicketTransportAccompagnateurs ? `${formatMontant(row.CoutTicketTransportAccompagnateurs)} €` : '—' },
...
export function buildRecapIntervenant(row) {
  ...
  return [
    ...
    { label: "Nom de l'intervenant.e", value: row.IntervenantNom },
    { label: "Email de l'intervenant.e", value: row.IntervenantEmail },
    { label: "Téléphone de l'intervenant.e", value: row.IntervenantTelephone },
    { label: "Activité / fonction de l'intervenant.e", value: row.IntervenantActivite },
