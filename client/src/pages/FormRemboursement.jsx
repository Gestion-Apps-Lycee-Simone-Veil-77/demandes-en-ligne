...
export default function FormRemboursement() {
  const { api } = useShell();
  ...
  const [f, setF] = useState({
    service: '', intitule: '', discussion: '', typeDepense: '', typeDepenseAutre: '', dateLimite: '',
    montant: '', fournisseur: '', contactFournisseur: '', fournisseurReference: '', complement: ''
  });
  const set = k => e => setF({ ...f, [k]: e.target.value });

  useEffect(() => { api.get('/bootstrap/remboursement').then(setBootstrap); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleSubmit(e) {
    ...
    if (f.typeDepense === 'Autre' && !f.typeDepenseAutre.trim()) {
      setStatus({ type: 'error', text: 'Merci de préciser le type de dépense.' }); return;
    }
    ...
    try {
      ...
      const res = await api.post('/submit/remboursement', {
        ...f,
      ...
      setF({ service: '', intitule: '', discussion: '', typeDepense: '', typeDepenseAutre: '', dateLimite: '', montant: '', fournisseur: '', contactFournisseur: '', fournisseurReference: '', complement: '' });
      setFile(null);
      e.target.reset();
    } catch (err) {
  ...
  return (
    <Layout back="/">
      <h2 className="page-title">Remboursement d'avance de frais</h2>
      <p className="mb-4 text-sm leading-relaxed text-slate-500">
        Ce formulaire permet de demander le remboursement d'une dépense que vous avez avancée personnellement pour le lycée.<br />
        Une pièce justificative doit être obligatoirement jointe. Votre demande sera examinée par le proviseur et la gestionnaire avant validation.
      </p>
      ...
      {bootstrap && (
        <form onSubmit={handleSubmit}>
          ...
          <Field label="Intitulé de la dépense" required hint="Nommez et expliquez pourquoi vous avez été amené(e) à avancer cette dépense.">
          ...
          {f.typeDepense === 'Autre' && (
            <Field label="Précisez le type de dépense" required>
              <input className={inputClass} required value={f.typeDepenseAutre} onChange={set('typeDepenseAutre')} />
            </Field>
          )}

          <Field label="Délai de dépense (date)" required>
            <input type="date" className={inputClass} min={today} required value={f.dateLimite} onChange={set('dateLimite')} />
          </Field>

          <Field label="Montant" required hint="En euros — la virgule est acceptée.">
            <input className={inputClass} inputMode="decimal" placeholder="Ex : 150,00" required value={f.montant} onChange={set('montant')} />
          </Field>
